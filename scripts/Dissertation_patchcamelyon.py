...
def apply_transforms(image):
    ...
    image = transforms.ColorJitter(brightness=0.3, contrast=0.4, saturation=0.3, hue=0.1)(image)
...
if __name__ == '__main__':
    ...
    for model_name in model_names:
        ...
        plt.figure()
        plt.plot(range(len(train_losses)), train_losses, label='Training Loss', linestyle='-')  # Training set loss curve

        plt.plot(range(len(val_losses)), val_losses, label='Validation Loss', linestyle='--')  # validation set loss curve
        
        plt.xlabel('Epoch', fontsize=14)  # X-axis labels
        plt.ylabel('Loss', fontsize=14)  # Y-axis labels
        plt.title(f'{model_name} Training and Validation Loss', fontsize=14)  # title
        
        plt.legend(fontsize=12)  # legend
        plt.xticks(fontsize=12)  # X-axis scale font size
        plt.yticks(fontsize=12)  # Y-axis scale font size
    ...
    for model_name in model_names:
        ...
        plt.figure()
        plt.plot(range(len(train_accuracies)), train_accuracies, label='Training Accuracy', linestyle='-')  # Training set accuracy curve

        plt.plot(range(len(val_accuracies)), val_accuracies, label='Validation Accuracy', linestyle='--')  # Training set accuracy curve
        
        plt.xlabel('Epoch', fontsize=14) # X-axis labels
        plt.ylabel('Accuracy', fontsize=14)  # Y-axis labels
        plt.title(f'{model_name} Training and Validation Accuracy', fontsize=14)  # title
   ...
   # figure2: Model AUC Comparison
    ...
    # figure 8: Model Accuracy, FLOPs and number of Parameters Comparison (Bubble Chart)
    fig, ax8 = plt.subplots()

    scatter = ax8.scatter(accuracies, inf_times, s=[param / 1e4 for param in params], alpha=0.7, c='gray') # Draw a bubble chart
    ...
    ax8.set_title('Model Accuracy, Inference Time, Parameters number comparison)') # plot title
    ax8.grid(True)
    
    # Add a legend
    legend1 = ax8.legend(['Size of bubble: number of Parameters'], loc='center right', bbox_to_anchor=(0.98, 0.5))
    ...
    # Save the chart
    plt.savefig('model_accuracy_inferencetime_Flops.png', bbox_inches='tight')
