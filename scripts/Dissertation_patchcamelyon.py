...
#  定义show_images函数，用于展示和保存图像
def show_images(images, title, filename):
    fig, axes = plt.subplots(4, 5, figsize=(20, 10))  # 调整为4行5列以适应20张图像
...
# 随机选择20张训练集和验证集的图像进行对比,并分类展示0和1的图像
# Randomly select 10 images from the training set and validation set for comparison, and classify and display images of 0 and 1
